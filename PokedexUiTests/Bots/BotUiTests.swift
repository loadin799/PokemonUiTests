...
//  PokedexGo
//
//  Created by Herbert Pereira Soares on 19/04/19.
...
open class Bot {
    ...
    public func tapLabel(text: String) -> Self  {
        XCTContext.runActivity(named: "Tap at Label with text: \(text)") { _ in
            ...
            let labelText = app.staticTexts.element(matching: predicate)
            let buttonLabelText = app.buttons.element(matching: predicate)
            let labelTextOnTable = app.tables.staticTexts.element(matching: predicate)
            if labelText.exists {
                labelText.tap()
            } else if buttonLabelText.exists {
                buttonLabelText.tap()
            }else if labelTextOnTable.exists{
                labelTextOnTable.tap()
            }
            else {
